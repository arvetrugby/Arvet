document.addEventListener('DOMContentLoaded', async function() {
  ...
  const emailActualSpan = document.getElementById('emailActual');
  if (emailActualSpan) {
    emailActualSpan.textContent = document.getElementById('email').value || 'No disponible';
  }
  ...
  document.getElementById('btnCambiarEmail')?.addEventListener('click', async function() {
    const nuevoEmail = document.getElementById('nuevoEmail').value.trim().toLowerCase();
    const emailActual = document.getElementById('email').value;
    
    // Validaciones
    if (!nuevoEmail) {
      mostrarMensaje('❌ Ingresá un email', 'error');
      return;
    }
    
    if (nuevoEmail === emailActual) {
      mostrarMensaje('❌ El email es igual al actual', 'error');
      return;
    }
    ...
    try {
      ...
      document.getElementById('emailActual').textContent = nuevoEmail;
      document.getElementById('nuevoEmail').value = '';
      
      // Actualizar localStorage si es el usuario logueado
      if (!esAdminEditando && user) {
        const updatedUser = { ...user, email: nuevoEmail };
        localStorage.setItem('arvet_user', JSON.stringify(updatedUser));
      }
      
      mostrarMensaje('✅ Usuario (email) actualizado');
      
    } catch (err) {
      console.error('Error:', err);
      mostrarMensaje('❌ Error al actualizar email', 'error');
    } finally {
      this.textContent = '💾 Actualizar email';
      this.disabled = false;
    }
  });
  // ==========================================
